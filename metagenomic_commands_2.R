...
freqs <- function(title,remove,freq){

title <- title
remove <- remove
frequency <- as.numeric(freq)
r <- grep(".bracken.txt",dir(),value=T) ; 
r1 <- data.frame() ; r2 <- 0 ; r3 <- 0 ; r4 <- 0 ; r5 <- 0 ; r6 <- 0 ; r7 <- 0 ; 
r8 <- 0 ; r8.1 <- 0 ; 
...
r3 <- append(r3, r2)
r4 <- append(r4, r1$name)
r5 <- append(r5, r1$new_est_reads)
r6 <- append(r6, r1$fraction_total_reads)
}

#check the items# 
r8 <- data.frame(sample=r3,species=r4,reads=r5,frequency=r6)
r8.1 <- r8[!r8$species %in% remove & r8$frequency > frequency , ]

#estimate percentages#
s1 <- 0 ; s2 <- 0 ; s3 <- 0 ; s4 <- 0 ; 
for (i in unique(r8.1$sample)){
s1 <- r8.1[r8.1$sample %in% i, 3]
s2 <- sum(r8.1[r8.1$sample %in% i, 3])
s3 <- (s1/s2)*100
s4 <- append(s4,s3)
}
...
write.csv(r8.1,paste0(title,".csv"),row.names=F)
...
heatmap_plot1 <- ggplot(data3, aes(x = reorder(species,-abundance), y = sample, fill = percentage)) +
		...
		scale_x_discrete(name=paste0(type," species (> ",reads,"reads)")) +
